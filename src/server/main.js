...
const API_KEY = process.env.API_KEY //export your own apiKey;
...
app.get('/apiUrlKey', (req, res) => {
  res.send({ url: mapboxUrl, key: API_KEY })
})
app.get('/map', (req, res) => {
  const { x, y, z } = req.query
  console.log('maboxUrls', mapboxUrl, x, y, z)
  const link = `${mapboxUrl}/${z}/${x}/${y}@2x?access_token=${API_KEY}`
  axios
    .get(link)
    .then((response) => {
      res.send(response.data)
